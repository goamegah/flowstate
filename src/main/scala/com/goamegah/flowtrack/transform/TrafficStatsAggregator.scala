...
object TrafficStatsAggregator {
  ...
  def avgSpeedByMaxSpeedAndStatus(df: DataFrame): DataFrame = {
    df
      .filter(col("average_speed").isNotNull && col("max_speed").isNotNull && col("traffic_status").isNotNull)
      .groupBy(col("max_speed"), col("traffic_status"))
      .agg(
        avg(col("average_speed").cast("double")).as("avg_speed"),
        count("*").as("record_count"),
        min(col("average_speed")).as("min_avg_speed"),
        max(col("average_speed")).as("max_avg_speed")
      )
  }
